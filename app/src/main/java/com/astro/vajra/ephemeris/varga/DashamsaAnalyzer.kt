...
object DashamsaAnalyzer {
    ...
    private fun determineCareerType(d10: com.astro.vajra.ephemeris.DivisionalChartData, lang: Language): List<CareerType> {
    ...
    private fun getCareerTypeFromPlanet(p: Planet, lang: Language): CareerType = when (p) {
        ...
        Planet.RAHU -> CareerType(StringResources.get(StringKeyDivisional.DASHAMSA_TYPE_UNCONVENTIONAL, lang), listOf(StringKeyDivisional.HORA_SUN_SOURCE_TECHNOLOGY, StringKeyDivisional.HORA_SUN_SOURCE_FOREIGN, StringKeyDivisional.HORA_SUN_SOURCE_RESEARCH, StringKeyAnalysis.PRASHNA_CAT_GENERAL).map { StringResources.get(it, lang) }, StringResources.get(StringKeyDivisional.DASHAMSA_SUIT_UNCONVENTIONAL, lang))
        Planet.KETU -> CareerType(StringResources.get(StringKeyDivisional.DASHAMSA_TYPE_SPIRITUAL, lang), listOf(StringKeyDivisional.HORA_SUN_SOURCE_SPIRITUAL, StringKeyDivisional.HORA_SUN_SOURCE_OCCULT, StringKeyDivisional.HORA_SUN_SOURCE_RESEARCH, StringKeyAnalysis.PRASHNA_CAT_HEALTH).map { StringResources.get(it, lang) }, StringResources.get(StringKeyDivisional.DASHAMSA_SUIT_SPIRITUAL, lang))
    ...
    private fun mapPlanetsToIndustries(d10: com.astro.vajra.ephemeris.DivisionalChartData, lang: Language): Map<Planet, List<String>> {
    ...
    private fun getIndustriesForPlanet(p: Planet, lang: Language): List<String> = when (p) {
        ...
        Planet.MARS -> listOf(StringKeyDivisional.NAVAMSA_PROF_MILITARY, StringKeyMatch.ACTIVITY_BUSINESS_NAME, StringKeyDivisional.NAVAMSA_PROF_MEDICINE, StringKeyDivisional.NAVAMSA_PROF_ENGINEERING, StringKeyDivisional.HORA_SUN_SOURCE_REAL_ESTATE)
        Planet.MERCURY -> listOf(StringKeyDivisional.NAVAMSA_PROF_BUSINESS, StringKeyMatch.PRASHNA_CAT_FINANCE, StringKeyDivisional.NAVAMSA_PROF_WRITING, StringKeyDivisional.NAVAMSA_PROF_TECH, StringKeyDivisional.HORA_SUN_SOURCE_COMMUNICATION)
        ...
        Planet.RAHU -> listOf(StringKeyDivisional.HORA_SUN_SOURCE_TECHNOLOGY, StringKeyDivisional.HORA_SUN_SOURCE_TRADE, StringKeyMatch.ACTIVITY_TRAVEL_NAME, StringKeyDivisional.HORA_SUN_SOURCE_LIQUIDS)
        Planet.KETU -> listOf(StringKeyDivisional.HORA_SUN_SOURCE_SPIRITUAL, StringKeyDivisional.HORA_SUN_SOURCE_OCCULT, StringKeyMatch.ACTIVITY_MEDICAL_NAME, StringKeyDivisional.HORA_SUN_SOURCE_RESEARCH)
        else -> emptyList()
    }.map { StringResources.get(it, lang) }

    private fun analyzeGovernmentServicePotential(sun: PlanetPosition?, d10: com.astro.vajra.ephemeris.DivisionalChartData, lang: Language): GovernmentServiceAnalysis {
    ...
    private fun calculateCareerPeakTiming(chart: VedicChart, d10: com.astro.vajra.ephemeris.DivisionalChartData, lang: Language): List<CareerPeakPeriod> {
    ...
    private fun analyzeMultipleCareers(d10: com.astro.vajra.ephemeris.DivisionalChartData, lang: Language): List<String> {
    ...
    private fun analyzeProfessionalStrengths(d10: com.astro.vajra.ephemeris.DivisionalChartData, lang: Language): List<String> {
