...
object GenericVargaAnalyzer {
    ...
    fun analyzeVarga(
        ...
        val houseSummaries = (1..12).map { houseNum ->
            ...
            HouseAnalysisSummary(
                ...
                status = if (planetsInHouse.isNotEmpty()) 
                    StringResources.get(com.astro.vajra.core.common.StringKeyAnalysis.DIVISIONAL_HOUSE_OCCUPIED, language) 
                else 
                    StringResources.get(com.astro.vajra.core.common.StringKeyAnalysis.DIVISIONAL_HOUSE_EMPTY, language),
                houseLord = houseLord
            )
        }
        ...
        if (strengthScore < 40) {
            recommendations.add(StringResources.get(com.astro.vajra.core.common.StringKeyAnalysis.DIVISIONAL_REMEDIES_DESC, language, type.shortName))
        }
        if (vargottamas.isNotEmpty()) {
            recommendations.add(StringResources.get(com.astro.vajra.core.common.StringKeyAnalysis.DIVISIONAL_STRENGTH_DESC, language, vargottamas.joinToString { it.getLocalizedName(language) }))
        }
