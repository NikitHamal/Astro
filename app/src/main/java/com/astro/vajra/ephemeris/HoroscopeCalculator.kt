...
class HoroscopeCalculator @Inject constructor(
    ...
    private fun getGocharaInfluenceKey(planet: Planet, house: Int, isFavorable: Boolean): Pair<com.astro.vajra.core.common.StringKeyInterface, Int> {
        // Special mapping to keys in StringKeyHoroscope
        val keyName = "GOCHARA_${planet.name}_$house"
        val key = try {
            com.astro.vajra.core.common.StringKeyHoroscope.valueOf(keyName)
        } catch (e: Exception) {
    ...
    private fun calculateLifeAreaPredictions(
        ...
        return LifeArea.entries.map { area ->
            ...
            val predictionKey = try {
                com.astro.vajra.core.common.StringKeyHoroscope.valueOf(predictionKeyName)
            } catch (e: Exception) {
    ...
    private fun calculateLuckyElements(
        ...
        val luckyColorKey = try {
            com.astro.vajra.core.common.StringKeyHoroscope.valueOf("LUCKY_COLOR_${moonSign.element.uppercase()}")
        } catch (e: Exception) {
        ...
        val luckyDirectionKey = try {
            com.astro.vajra.core.common.StringKeyHoroscope.valueOf("LUCKY_DIR_${PLANET_DIRECTIONS_MAP[ascRuler] ?: "EAST"}")
        } catch (e: Exception) {
    ...
    private fun generateWeeklyAdvice(
        ...
        keyDates.firstOrNull { it.isPositive }?.let {
            val dateStr = if (language == Language.NEPALI) com.astro.vajra.core.common.BikramSambatConverter.toNepaliNumerals(it.date.dayOfMonth.toString())
