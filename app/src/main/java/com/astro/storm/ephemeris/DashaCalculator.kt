...
object DashaCalculator {
    ...
    data class Pranadasha(
        ...
        fun getLocalizedDurationString(language: Language): String {
            ...
            val hours = totalMins / 60
            val mins = totalMins % 60
            val daysShort = StringResources.get(StringKeyGeneralPart3.DAYS_SHORT, language)
            val hoursShort = StringResources.get(StringKeyGeneralPart5.HOURS_SHORT, language)
            val minutesShort = StringResources.get(StringKeyGeneralPart7.MINUTES_SHORT, language)
            return when {
                hours >= 24 -> {
                    val days = hours / 24
                    val remainingHours = hours % 24
    ...
    enum class DashaLevel(val displayName: String, val levelNumber: Int) {
        ...
        fun getLocalizedName(language: Language): String = when (this) {
            MAHADASHA -> StringResources.get(StringKeyDashaPart1.DASHA_MAHADASHA, language)
            ANTARDASHA -> StringResources.get(StringKeyDashaPart1.DASHA_ANTARDASHA, language)
            PRATYANTARDASHA -> StringResources.get(StringKeyDashaPart1.DASHA_PRATYANTARDASHA, language)
            SOOKSHMADASHA -> StringResources.get(StringKeyDashaPart1.DASHA_SOOKSHMADASHA, language)
            PRANADASHA -> StringResources.get(StringKeyDashaPart1.DASHA_PRANADASHA, language)
            DEHADASHA -> StringResources.get(StringKeyDashaPart1.DASHA_DEHADASHA, language)
        }
    }
        ...
        fun getLocalizedCurrentPeriodDescription(language: Language): String {
            return buildString {
                currentMahadasha?.let { md ->
                    ...
                    currentAntardasha?.let { ad ->
                        append(" → ${ad.planet.getLocalizedName(language)} ${StringResources.get(StringKeyDashaPart1.DASHA_BHUKTI, language)}")
                        currentPratyantardasha?.let { pd ->
                            append(" → ${pd.planet.getLocalizedName(language)} ${StringResources.get(StringKeyDashaPart1.DASHA_PRATYANTAR, language)}")
                            currentSookshmadasha?.let { sd ->
                                append(" → ${sd.planet.getLocalizedName(language)} ${StringResources.get(StringKeyDashaPart1.DASHA_SOOKSHMA, language)}")
                            }
                        }
                    }
                } ?: append(StringResources.get(StringKeyDashaPart1.DASHA_NO_ACTIVE_PERIOD, language))
            }
        }
                    ...
                    currentAntardasha?.let { ad ->
                        ...
                        currentPratyantardasha?.let { pd ->
                            ...
                            currentSookshmadasha?.let { sd ->
                                append(" → ${sd.planet.getLocalizedName(language)} ${StringResources.get(StringKeyDashaPart1.DASHA_SOOKSHMA, language)}")
                                currentPranadasha?.let { prd ->
                                    append(" → ${prd.planet.getLocalizedName(language)} ${StringResources.get(StringKeyDashaPart1.DASHA_PRANA, language)}")
                                    currentDehadasha?.let { dd ->
                                        append(" → ${dd.planet.getLocalizedName(language)} ${StringResources.get(StringKeyDashaPart1.DASHA_DEHA, language)}")
                                    }
                                }
                            }
                        }
    ...
    fun formatLocalizedDashaPeriod(mahadasha: Mahadasha, language: Language): String {
        ...
        val durationLabel = StringResources.get(StringKeyDashaPart1.DASHA_DURATION, language)
        val periodLabel = StringResources.get(StringKeyDashaPart1.DASHA_PERIOD, language)
        val yearsLabel = StringResources.get(StringKeyGeneralPart12.YEARS, language)
        val toLabel = StringResources.get(StringKeyGeneralPart11.TO, language)
        val statusLabel = StringResources.get(StringKeyDashaPart1.DASHA_STATUS, language)
        val activeLabel = StringResources.get(StringKeyDashaPart1.DASHA_CURRENTLY_ACTIVE, language)
        val progressLabel = StringResources.get(StringKeyDashaPart1.DASHA_PROGRESS, language)
        val remainingLabel = StringResources.get(StringKeyDashaPart1.DASHA_REMAINING, language)
    ...
    fun formatLocalizedCurrentPeriod(timeline: DashaTimeline, language: Language): String {
        ...
        val durationLabel = StringResources.get(StringKeyDashaPart1.DASHA_DURATION, language)
        val yearsLabel = StringResources.get(StringKeyGeneralPart12.YEARS, language)
        val daysLabel = StringResources.get(StringKeyGeneralPart3.DAYS, language)
        val toLabel = StringResources.get(StringKeyGeneralPart11.TO, language)
        ...
        return buildString {
            ...
            } ?: appendLine(StringResources.get(StringKeyDashaPart1.DASHA_NO_ACTIVE_PERIOD, language))
        }
    }
}
...
object ConditionalDashaCalculator {
    ...
    enum class Yogini(
        ...
        fun getLocalizedName(language: Language): String = when (this) {
            MANGALA -> StringResources.get(StringKeyDashaPart3.YOGINI_MANGALA, language)
            PINGALA -> StringResources.get(StringKeyDashaPart3.YOGINI_PINGALA, language)
            DHANYA -> StringResources.get(StringKeyDashaPart3.YOGINI_DHANYA, language)
            BHRAMARI -> StringResources.get(StringKeyDashaPart2.YOGINI_BHRAMARI, language)
            BHADRIKA -> StringResources.get(StringKeyDashaPart2.YOGINI_BHADRIKA, language)
            ULKA -> StringResources.get(StringKeyDashaPart3.YOGINI_ULKA, language)
            SIDDHA -> StringResources.get(StringKeyDashaPart3.YOGINI_SIDDHA, language)
            SANKATA -> StringResources.get(StringKeyDashaPart3.YOGINI_SANKATA, language)
        }

        fun getLocalizedDeity(language: Language): String = when (this) {
            MANGALA -> StringResources.get(StringKeyDashaPart3.YOGINI_DEITY_CHANDRA, language)
            PINGALA -> StringResources.get(StringKeyDashaPart3.YOGINI_DEITY_SURYA, language)
            DHANYA -> StringResources.get(StringKeyDashaPart3.YOGINI_DEITY_GURU, language)
            BHRAMARI -> StringResources.get(StringKeyDashaPart3.YOGINI_DEITY_MANGAL, language)
            BHADRIKA -> StringResources.get(StringKeyDashaPart3.YOGINI_DEITY_BUDHA, language)
            ULKA -> StringResources.get(StringKeyDashaPart3.YOGINI_DEITY_SHANI, language)
            SIDDHA -> StringResources.get(StringKeyDashaPart3.YOGINI_DEITY_SHUKRA, language)
            SANKATA -> StringResources.get(StringKeyDashaPart3.YOGINI_DEITY_RAHU, language)
        }
    }
    ...
    enum class YoginiNature(val displayName: String) {
        ...
        fun getLocalizedName(language: Language): String = when (this) {
            BENEFIC -> StringResources.get(StringKeyGeneralPart7.NATURE_BENEFIC, language)
            MALEFIC -> StringResources.get(StringKeyGeneralPart7.NATURE_MALEFIC, language)
            MIXED -> StringResources.get(StringKeyGeneralPart7.NATURE_MIXED, language)
        }
    }
