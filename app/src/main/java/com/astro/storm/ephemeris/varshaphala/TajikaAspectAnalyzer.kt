...
object TajikaAspectAnalyzer {
    ...
    private fun getAspectEffectDescription(type: TajikaAspectType, p1: Planet, p2: Planet, language: Language): String {
        return when (type) {
            TajikaAspectType.ITHASALA -> StringResources.get(StringKeyVarshaphala.TAJIKA_ITHASALA_EFFECT, language, p1.getLocalizedName(language), p2.getLocalizedName(language))
            TajikaAspectType.EASARAPHA -> StringResources.get(StringKeyVarshaphala.TAJIKA_EASARAPHA_EFFECT, language)
