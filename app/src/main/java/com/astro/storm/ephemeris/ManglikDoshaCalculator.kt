...
object ManglikDoshaCalculator {
    ...
    enum class ManglikLevel {
        ...
        fun getLocalizedName(language: Language): String = when (this) {
            NONE -> StringResources.get(StringKeyMatchmakingPart1.MANGLIK_NONE_LEVEL, language)
            MILD -> StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MILD, language)
            PARTIAL -> StringResources.get(StringKeyMatchmakingPart1.MANGLIK_PARTIAL_LEVEL, language)
            FULL -> StringResources.get(StringKeyMatchmakingPart1.MANGLIK_FULL_LEVEL, language)
            SEVERE -> StringResources.get(StringKeyMatchmakingPart1.MANGLIK_SEVERE, language)
        }
    }
    ...
    data class ManglikAnalysis(
        ...
        fun getSummary(language: Language): String {
            return if (isManglik && effectiveLevel != ManglikLevel.NONE) {
                ...
                StringResources.get(StringKeyMatchmakingPart1.MANGLIK_SUMMARY_PRESENT, language)
                    .replace("{level}", levelName)
                    .replace("{intensity}", "%.1f".format(remainingIntensityAfterCancellations))
            } else {
                StringResources.get(StringKeyMatchmakingPart1.MANGLIK_SUMMARY_ABSENT, language)
            }
        }
        ...
        fun getInterpretation(language: Language): String {
            return buildString {
                if (!isManglik || effectiveLevel == ManglikLevel.NONE) {
                    appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_INTERP_NO_DOSHA_TITLE, language))
                    appendLine()
                    appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_INTERP_NO_DOSHA_DESC, language))
                    return@buildString
                }

                appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_INTERP_TITLE, language))
                appendLine()

                marsPosition?.let {
                    appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_INTERP_MARS_POSITION, language)
                        .replace("{sign}", it.sign.getLocalizedName(language))
                        .replace("{house}", it.house.toString()))
                    if (isMarsRetrograde) {
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_INTERP_MARS_RETROGRADE, language))
                    }
                    appendLine()
                }

                appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_INTERP_THREE_REF, language))
                appendLine()

                appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_FROM_LAGNA, language) + " (${analysisFromLagna.referenceSign.getLocalizedName(language)}):")
                appendLine("  ${StringResources.get(StringKeyMatchmakingPart1.MANGLIK_INTERP_MARS_HOUSE, language).replace("{house}", analysisFromLagna.marsHouse.toString())}")
                appendLine("  ${StringResources.get(StringKeyMatchmakingPart1.MANGLIK_INTERP_IS_MANGLIK, language)}: ${if (analysisFromLagna.isManglik) StringResources.get(StringKeyGeneralPart12.YES, language) else StringResources.get(StringKeyGeneralPart7.NO, language)}")
                appendLine()

                appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_FROM_MOON, language) + " (${analysisFromMoon.referenceSign.getLocalizedName(language)}):")
                appendLine("  ${StringResources.get(StringKeyMatchmakingPart1.MANGLIK_INTERP_MARS_HOUSE, language).replace("{house}", analysisFromMoon.marsHouse.toString())}")
                appendLine("  ${StringResources.get(StringKeyMatchmakingPart1.MANGLIK_INTERP_IS_MANGLIK, language)}: ${if (analysisFromMoon.isManglik) StringResources.get(StringKeyGeneralPart12.YES, language) else StringResources.get(StringKeyGeneralPart7.NO, language)}")
                appendLine()

                appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_FROM_VENUS, language) + " (${analysisFromVenus.referenceSign.getLocalizedName(language)}):")
                appendLine("  ${StringResources.get(StringKeyMatchmakingPart1.MANGLIK_INTERP_MARS_HOUSE, language).replace("{house}", analysisFromVenus.marsHouse.toString())}")
                appendLine("  ${StringResources.get(StringKeyMatchmakingPart1.MANGLIK_INTERP_IS_MANGLIK, language)}: ${if (analysisFromVenus.isManglik) StringResources.get(StringKeyGeneralPart12.YES, language) else StringResources.get(StringKeyGeneralPart7.NO, language)}")
                appendLine()

                appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_INTERP_INITIAL_LEVEL, language).replace("{level}", level.getLocalizedName(language)))

                if (cancellationFactors.isNotEmpty()) {
                    appendLine()
                    appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_CANCELLATIONS, language) + ":")
                    cancellationFactors.forEach { factor ->
                        appendLine("  - ${factor.getTitle(language)} (${factor.strength.name})")
                    }
                    appendLine()
                    appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_EFFECTIVE_LEVEL, language) + ": ${effectiveLevel.getLocalizedName(language)}")
                }
            }
        }
        ...
        fun getMarriageConsiderations(language: Language): String {
            return buildString {
                appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_TITLE, language))
                appendLine()

                when (effectiveLevel) {
                    ManglikLevel.NONE -> {
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_NONE_1, language))
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_NONE_2, language))
                    }
                    ManglikLevel.MILD -> {
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_MILD_1, language))
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_MILD_2, language))
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_MILD_3, language))
                    }
                    ManglikLevel.PARTIAL -> {
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_PARTIAL_1, language))
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_PARTIAL_2, language))
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_PARTIAL_3, language))
                    }
                    ManglikLevel.FULL -> {
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_FULL_1, language))
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_FULL_2, language))
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_FULL_3, language))
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_FULL_4, language))
                    }
                    ManglikLevel.SEVERE -> {
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_SEVERE_1, language))
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_SEVERE_2, language))
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_SEVERE_3, language))
                        appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_SEVERE_4, language))
                    }
                }

                if (cancellationFactors.any { it.strength == CancellationStrength.FULL }) {
                    appendLine()
                    appendLine(StringResources.get(StringKeyMatchmakingPart1.MANGLIK_MARRIAGE_FULL_CANCEL_NOTE, language))
                }
            }
        }
    }
    ...
    fun calculateManglikDosha(chart: VedicChart): ManglikAnalysis {
        ...
        val analysisFromLagna = analyzeFromReference(
            StringKeyMatchmakingPart1.MANGLIK_REF_LAGNA,
            ascendantSign,
            marsPosition
        )

        val analysisFromMoon = analyzeFromReference(
            StringKeyMatchmakingPart1.MANGLIK_REF_MOON,
            moonPosition?.sign ?: ascendantSign,
            marsPosition
        )

        val analysisFromVenus = analyzeFromReference(
            StringKeyMatchmakingPart1.MANGLIK_REF_VENUS,
    ...
    private fun findCancellationFactors(
        ...
        if (marsPosition.sign == ZodiacSign.ARIES || marsPosition.sign == ZodiacSign.SCORPIO) {
            factors.add(CancellationFactor(
                titleKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_OWN_SIGN_TITLE,
                descriptionKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_OWN_SIGN_DESC,
        ...
        if (marsPosition.sign == ZodiacSign.CAPRICORN) {
            factors.add(CancellationFactor(
                titleKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_EXALTED_TITLE,
                descriptionKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_EXALTED_DESC,
        ...
        if (jupiter != null && jupiter.house == marsPosition.house) {
            factors.add(CancellationFactor(
                titleKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_JUPITER_CONJUNCT_TITLE,
                descriptionKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_JUPITER_CONJUNCT_DESC,
                strength = CancellationStrength.FULL
            ))
        }
        ...
        if (venus != null && venus.house == marsPosition.house) {
            factors.add(CancellationFactor(
                titleKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_VENUS_CONJUNCT_TITLE,
                descriptionKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_VENUS_CONJUNCT_DESC,
                strength = CancellationStrength.STRONG
            ))
        }
        ...
        if (jupiter != null) {
            ...
            if (marsPosition.house in jupiterAspects) {
                factors.add(CancellationFactor(
                    titleKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_JUPITER_ASPECT_TITLE,
                    descriptionKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_JUPITER_ASPECT_DESC,
                    strength = CancellationStrength.STRONG
                ))
            }
        }
        ...
        if (analysisFromLagna.marsHouse == 2 &&
            ...
            factors.add(CancellationFactor(
                titleKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_SECOND_MERCURY_TITLE,
                descriptionKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_SECOND_MERCURY_DESC,
                strength = CancellationStrength.FULL
            ))
        }
        ...
        if (analysisFromLagna.marsHouse == 4 &&
            ...
            factors.add(CancellationFactor(
                titleKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_FOURTH_OWN_TITLE,
                descriptionKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_FOURTH_OWN_DESC,
                strength = CancellationStrength.FULL
            ))
        }
        ...
        if (analysisFromLagna.marsHouse == 7 &&
            ...
            factors.add(CancellationFactor(
                titleKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_SEVENTH_SPECIAL_TITLE,
                descriptionKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_SEVENTH_SPECIAL_DESC,
                strength = CancellationStrength.STRONG
            ))
        }
        ...
        if (analysisFromLagna.marsHouse == 8 &&
            ...
            factors.add(CancellationFactor(
                titleKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_EIGHTH_JUPITER_TITLE,
                descriptionKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_EIGHTH_JUPITER_DESC,
                strength = CancellationStrength.STRONG
            ))
        }
        ...
        if (analysisFromLagna.marsHouse == 12 &&
            ...
            factors.add(CancellationFactor(
                titleKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_TWELFTH_VENUS_TITLE,
                descriptionKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_TWELFTH_VENUS_DESC,
                strength = CancellationStrength.FULL
            ))
        }
        ...
        if (ascendantSign in listOf(ZodiacSign.ARIES, ZodiacSign.CANCER, ZodiacSign.LEO, ZodiacSign.SCORPIO)) {
            factors.add(CancellationFactor(
                titleKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_BENEFIC_ASC_TITLE,
                descriptionKey = StringKeyMatchmakingPart1.MANGLIK_CANCEL_BENEFIC_ASC_DESC,
    ...
    fun checkManglikCompatibility(
        ...
        val recommendationKey = when (compatibility) {
            CompatibilityLevel.EXCELLENT -> StringKeyMatchmakingPart1.MANGLIK_COMPAT_EXCELLENT
            CompatibilityLevel.GOOD -> StringKeyMatchmakingPart1.MANGLIK_COMPAT_GOOD
            CompatibilityLevel.AVERAGE -> StringKeyMatchmakingPart1.MANGLIK_COMPAT_AVERAGE
            CompatibilityLevel.BELOW_AVERAGE -> StringKeyMatchmakingPart1.MANGLIK_COMPAT_BELOW_AVERAGE
            CompatibilityLevel.POOR -> StringKeyMatchmakingPart1.MANGLIK_COMPAT_POOR
        }
