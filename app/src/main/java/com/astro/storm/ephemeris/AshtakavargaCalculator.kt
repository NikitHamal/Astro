    ...
    data class AshtakavargaAnalysis(
        ...
        fun getLocalizedTransitScore(planet: Planet, sign: ZodiacSign, language: Language): TransitScore {
            val bav = bhinnashtakavarga[planet] ?: return TransitScore(
                ...
                binduScore = 0,
                savScore = sarvashtakavarga.getBindusForSign(sign),
                interpretation = StringResources.get(StringKeyUIPart1.LABEL_UNKNOWN, language)
            )
            ...
            val interpretation = when {
                binduScore >= 5 && savScore >= 30 -> StringResources.get(StringKeyTransitPart1.TRANSIT_INTERP_EXCELLENT, language)
                binduScore >= 4 && savScore >= 28 -> StringResources.get(StringKeyTransitPart1.TRANSIT_INTERP_GOOD, language)
                binduScore >= 3 && savScore >= 25 -> StringResources.get(StringKeyTransitPart1.TRANSIT_INTERP_AVERAGE, language)
                binduScore >= 2 && savScore >= 22 -> StringResources.get(StringKeyTransitPart1.TRANSIT_INTERP_BELOW_AVG, language)
                else -> StringResources.get(StringKeyTransitPart1.TRANSIT_INTERP_DIFFICULT, language)
            }
