...
class VarshaphalaCalculator(context: Context) {
    ...
    private fun getYearLordDignityDescription(planet: com.astro.storm.core.model.Planet, chart: SolarReturnChart, language: Language): String {
        ...
        return StringResources.get(StringKeyGeneralPart12.VARSHA_YEARLORD_DIGNITY_FORMAT, language, planet.getLocalizedName(language), pos.sign.getLocalizedName(language), pos.house)
    }
    ...
    private fun calculateKeyDates(chart: SolarReturnChart, solarReturnTime: LocalDateTime, muddaDasha: List<MuddaDashaPeriod>, language: Language): List<KeyDate> {
        val dates = mutableListOf(KeyDate(solarReturnTime.toLocalDate(), StringResources.get(StringKeyGeneralPart12.VARSHA_EVENT_SOLAR_RETURN, language), KeyDateType.IMPORTANT, StringResources.get(StringKeyGeneralPart12.VARSHA_EVENT_SOLAR_RETURN_DESC, language)))
        muddaDasha.forEach { dates.add(KeyDate(it.startDate, StringResources.get(StringKeyGeneralPart12.VARSHA_EVENT_DASHA_BEGINS, language, it.planet.getLocalizedName(language)), if (it.planetStrength in listOf(StringResources.get(StringKeyGeneralPart12.VARSHA_STRENGTH_EXALTED, language), StringResources.get(StringKeyGeneralPart12.VARSHA_STRENGTH_STRONG, language))) KeyDateType.FAVORABLE else KeyDateType.IMPORTANT, StringResources.get(StringKeyGeneralPart12.VARSHA_EVENT_DASHA_BEGINS_DESC, language, it.planet.getLocalizedName(language), it.days))) }
