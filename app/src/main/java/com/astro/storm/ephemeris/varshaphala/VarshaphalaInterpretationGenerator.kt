...
object VarshaphalaInterpretationGenerator {
    ...
    private fun generateHousePrediction(house: Int, sign: ZodiacSign, lord: Planet, lordPos: Int, planets: List<Planet>, chart: SolarReturnChart, muntha: MunthaResult, yearLord: Planet, language: Language): String {
        val s = evaluatePlanetStrengthDescription(lord, chart, language)
        val lordAnalysis = StringResources.get(StringKeyAnalysis.VARSHA_HOUSE_LORD_POSITION, language, lord.getLocalizedName(language), lordPos) + " " +
                when (s) {
        ...
        val influence = if (planets.isNotEmpty()) {
            ...
            when { benefics.isNotEmpty() && malefics.isEmpty() -> " " + StringResources.get(StringKeyAnalysis.VARSHA_HOUSE_BENEFICS_ENHANCE, language, benefics.joinToString { it.getLocalizedName(language) })
                malefics.isNotEmpty() && benefics.isEmpty() -> " " + StringResources.get(StringKeyAnalysis.VARSHA_HOUSE_MALEFICS_CHALLENGE, language, malefics.joinToString { it.getLocalizedName(language) })
                else -> " " + StringResources.get(StringKeyAnalysis.VARSHA_HOUSE_MIXED_INF, language, planets.joinToString { it.getLocalizedName(language) }) }
        } else " " + StringResources.get(StringKeyAnalysis.VARSHA_HOUSE_LORD_DEPENDENT, language)
        val special = (if (muntha.house == house) " " + StringResources.get(StringKeyAnalysis.VARSHA_HOUSE_MUNTHA_EMPHASIS, language) else "") +
                (if (yearLord == lord) " " + StringResources.get(StringKeyAnalysis.VARSHA_HOUSE_YEARLORD_RULE, language) else "")
        return StringResources.get(StringKeyAnalysis.VARSHA_HOUSE_PREDICTION_FORMAT, language, house, sign.getLocalizedName(language), getHouseSignificance(house, language), lordAnalysis, influence, special).trim()
    }
    ...
    private fun generateSpecificEvents(house: Int, lord: Planet, planets: List<Planet>, chart: SolarReturnChart, language: Language): List<String> {
        ...
        val s = evaluatePlanetStrengthDescription(lord, chart, language)
        val isLordStrong = s in listOf(StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_EXALTED, language), StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_STRONG, language))
        when (house) {
            1 -> { if (isLordStrong) { events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_VITALITY, language)); events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_NEW_VENTURES, language)) }
                if (Planet.JUPITER in planets) events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_SPIRITUAL_GROWTH, language))
                if (Planet.MARS in planets) events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_INCREASED_ENERGY, language)) }
            2 -> { if (isLordStrong) { events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_FINANCIAL_GAINS, language)); events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_FAMILY_RELATIONS, language)) }
                if (Planet.VENUS in planets) events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_LUXURY_ACQUISITION, language)) }
            5 -> { if (isLordStrong) { events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_CREATIVE_SUCCESS, language)); events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_CHILDREN_MATTERS, language)) }
                if (Planet.JUPITER in planets) events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_ACADEMIC_SUCCESS, language))
                if (Planet.VENUS in planets) events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_ROMANTIC_HAPPINESS, language)) }
            7 -> { if (isLordStrong) { events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_PARTNERSHIP_STRENGTH, language)); events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_MARRIAGE_FAVORABLE, language)) }
                if (Planet.VENUS in planets) events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_ROMANTIC_FULFILLMENT, language)) }
            10 -> { if (isLordStrong) { events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_CAREER_ADVANCEMENT, language)); events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_AUTHORITY_RECOGNITION, language)) }
                if (Planet.SUN in planets) events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_GOVERNMENT_FAVOR, language)) }
            11 -> { if (isLordStrong) { events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_DESIRE_FULFILLMENT, language)); events.add(StringResources.get(StringKeyAnalysis.VARSHA_EVENT_MULTIPLE_GAINS, language)) } }
        }
    ...
    fun identifyMajorThemes(chart: SolarReturnChart, muntha: MunthaResult, yearLord: Planet, housePredictions: List<HousePrediction>, triPataki: TriPatakiChakra, tajikaAspects: List<TajikaAspectResult>, language: Language): List<String> {
        ...
        housePredictions.filter { it.strength in listOf(StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_EXCELLENT, language), StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_STRONG, language)) }
            .sortedByDescending { it.rating }.take(2).forEach { themes.add(StringResources.get(StringKeyAnalysis.VARSHA_THEME_FAVORABLE, language, getHouseSignificance(it.house, language), it.house)) }
        val pos = tajikaAspects.count { it.type.isPositive }; val tot = tajikaAspects.size
        if (tot > 0) themes.add(StringResources.get(StringKeyAnalysis.VARSHA_THEME_TAJIKA, language, if (pos > tot / 2) StringResources.get(StringKeyAnalysis.VARSHA_TONE_SUPPORTIVE, language) else StringResources.get(StringKeyAnalysis.VARSHA_TONE_CHALLENGING, language), pos, tot))
    ...
    fun generateOverallPrediction(chart: SolarReturnChart, yearLord: Planet, muntha: MunthaResult, tajikaAspects: List<TajikaAspectResult>, housePredictions: List<HousePrediction>, language: Language): String {
        val strong = housePredictions.count { it.strength in listOf(StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_EXCELLENT, language), StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_STRONG, language)) }
        val weak = housePredictions.count { it.strength in listOf(StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_WEAK, language), StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_CHALLENGED, language)) }
        val s = evaluatePlanetStrengthDescription(yearLord, chart, language)
        val overallTone = when {
            s in listOf(StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_EXALTED, language), StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_STRONG, language)) && strong >= 6 -> StringResources.get(StringKeyAnalysis.VARSHA_TONE_EXCELLENT, language)
            s in listOf(StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_EXALTED, language), StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_STRONG, language)) && strong >= 4 -> StringResources.get(StringKeyAnalysis.VARSHA_TONE_FAVORABLE, language)
            strong > weak -> StringResources.get(StringKeyAnalysis.VARSHA_TONE_POSITIVE, language)
            weak > strong -> StringResources.get(StringKeyAnalysis.VARSHA_TONE_CHALLENGING_GROWTH, language)
            else -> StringResources.get(StringKeyAnalysis.VARSHA_TONE_BALANCED, language)
        }
        val ylInf = when (yearLord) { Planet.SUN -> StringResources.get(StringKeyAnalysis.VARSHA_YEARLORD_SUN, language); Planet.MOON -> StringResources.get(StringKeyAnalysis.VARSHA_YEARLORD_MOON, language); Planet.MARS -> StringResources.get(StringKeyAnalysis.VARSHA_YEARLORD_MARS, language); Planet.MERCURY -> StringResources.get(StringKeyAnalysis.VARSHA_YEARLORD_MERCURY, language); Planet.JUPITER -> StringResources.get(StringKeyAnalysis.VARSHA_YEARLORD_JUPITER, language); Planet.VENUS -> StringResources.get(StringKeyAnalysis.VARSHA_YEARLORD_VENUS, language); Planet.SATURN -> StringResources.get(StringKeyAnalysis.VARSHA_YEARLORD_SATURN, language); else -> StringResources.get(StringKeyAnalysis.VARSHA_YEARLORD_GENERAL, language) }
        val mTheme = muntha.themes.firstOrNull() ?: StringResources.get(StringKeyAnalysis.MUNTHA_GENERAL_GROWTH, language)
        val mInf = StringResources.get(StringKeyAnalysis.VARSHA_MUNTHA_INFLUENCE, language, muntha.house, muntha.sign.getLocalizedName(language), mTheme.lowercase())
        return StringResources.get(StringKeyAnalysis.VARSHA_OVERALL_TEMPLATE, language, overallTone.lowercase(), ylInf, mInf, tajikaAspects.count { it.type.isPositive }, tajikaAspects.count { !it.type.isPositive })
    }
    ...
    fun calculateYearRating(chart: SolarReturnChart, yearLord: Planet, muntha: MunthaResult, tajikaAspects: List<TajikaAspectResult>, housePredictions: List<HousePrediction>, language: Language): Float {
        var r = 3.0f
        val s = evaluatePlanetStrengthDescription(yearLord, chart, language)
        r += when (s) { StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_EXALTED, language) -> 0.8f; StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_STRONG, language) -> 0.5f; StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_ANGULAR, language) -> 0.3f; StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_DEBILITATED, language) -> -0.5f; else -> 0.0f }
        r += when (muntha.lordStrength) { StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_EXALTED, language), StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_STRONG, language) -> 0.3f; StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_MODERATE, language) -> 0.1f; StringResources.get(StringKeyAnalysis.VARSHA_STRENGTH_DEBILITATED, language) -> -0.3f; else -> 0.0f }
