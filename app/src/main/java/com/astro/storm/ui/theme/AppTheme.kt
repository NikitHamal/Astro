...
object AppTheme {
    ...
    fun getPlanetColor(planet: Planet): Color = when (planet) {
        Planet.SUN -> current.PlanetSun
        Planet.MOON -> current.PlanetMoon
        Planet.MARS -> current.PlanetMars
        Planet.MERCURY -> current.PlanetMercury
        Planet.JUPITER -> current.PlanetJupiter
        Planet.VENUS -> current.PlanetVenus
        Planet.SATURN -> current.PlanetSaturn
        Planet.RAHU -> current.PlanetRahu
        Planet.KETU -> current.PlanetKetu
        else -> current.AccentGold
    }
